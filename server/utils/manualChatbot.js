...
async function intialMoped(thread_id) {
    return [await openai.requestGPT("Im Folgenden werden Schritt für Schritt die Angaben des Benutzers ermittellt. Diese müssen sein: 'Vorname', 'Nachname', 'Adresse', 'Marke und Modell des Rollers', 'Vollkasko oder Teilkasko' 'Neuwert des Rollers' und 'Alter des jüngsten Fahrers'. Sollte der Benutzer einer der Attribute vergessen oder noch nicht im Laufe des Gesprächs ermittelt worden sein, dann fordere ihn auf, die fehlenden Daten nach und nach einzugeben. Überprüfe außerdem, ob die eingegebenen Daten realistisch sein können. Falls nicht, fordere den Benutzer auf, die Eingabe zu korrigieren. Sobald du alle Daten hast, gibst du sie als ein JSON-Dokument mit folgenden Attributnamen heraus: 'geschlecht', 'vorname', 'nachname', 'modell', 'neuwert', 'kasko', 'alter', 'plz', 'stadt', 'straße' und 'hausnummer'.", thread_id), false];
}

async function intialHaftpflicht(thread_id) {
    return [await openai.requestGPT("Im Folgenden werden Schritt für Schritt die Angaben des Benutzers ermittellt. Diese müssen sein: 'Vorname', 'Nachname', 'Adresse', 'Tarif' (Basis, Optimal, Plus), 'Öffentlicher Dienst', 'Ehrenamtliche Tätigkeit bei einer Blaulichtorganisation' und 'Single -oder Familien-Tarif'. Sollte der Benutzer einer der Attribute vergessen oder noch nicht im Laufe des Gesprächs ermittelt worden sein, dann fordere ihn auf, die fehlenden Daten nach und nach einzugeben. Überprüfe außerdem, ob die eingegebenen Daten realistisch sein können. Falls nicht, fordere den Benutzer auf, die Eingabe zu korrigieren. Sobald du alle Daten hast, gibst du sie als ein JSON-Dokument mit folgenden Attributnamen heraus: 'geschlecht', 'vorname', 'nachname', 'plz', 'stadt', 'straße', 'hausnummer', 'tarif', 'öffDienst', 'ehrenamt' und 'singleFamilie'.", thread_id), false];
}
